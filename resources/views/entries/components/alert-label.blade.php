     ...
     document.addEventListener("click", (e) => {
          if (e.target == addLabelBttn || e.target.matches(".add-bttn *")) {
               Swal.fire({
                    ...
                    html:
                         `<form action="{{ route("entries.add_label", $entry) }}" class="form-labels" method="post">
                              @csrf
                              @method("put")
                              ...
                              @foreach ($user->labels as $label)
                                   <div class="input-container alert-label-container">
                                        ...
                                        <input type="checkbox" name="labels[]" id="label-{{ $label->id }}" value="{{ $label->id }}" @if($entry->labels->contains($label)) checked @endif>
                                   </div>
                              @endforeach
